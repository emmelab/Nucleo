...
void draw() {
 // sistema.claseEstimulos.recibirUnEstimulo_CLASE("mouse", mouseX*1.0, mouseY*1.0);

  if (!pausa) {
    ciclo();
  }
//if(sistema.debug){
  ArrayList p = sistema.claseEstimulos.getPosicionDeEstimulos();
  for (int i=0; i<p.size (); i++) {
    PVector pos = (PVector)p.get(i);
   // fill(255);
   // stroke(255, 0, 255);
   // strokeWeight(3);
    ellipse(pos.x, pos.y, 10, 10);
    // println("estoy dibujando cosas locas"+pos.x+"__!!__"+pos.y);
    // println(categoria);
  }
//}
}
