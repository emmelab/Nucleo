...
class Mod_AtraccionAlCentro extends Modificador {
  ...
  void ejecutar(Sistema s) {
    ...
    for (int i=0; i<s.tamano; i++) {
      ...
      if (s.claseEstimulos.getEstimulo()==null) {
        ...
        acel.a[i].add(PVector.mult(PVector.fromAngle(s.p5.atan2(s.claseEstimulos.getEstimulo().y-p.y, s.claseEstimulos.getEstimulo().x-p.x)), s.tamano*factor*2.5f) );
      }
  ...
  static Registrador<Mod_AtraccionAlCentro> registrador = new Registrador() {
    public String key() {
      return "Atraccion Al Centro";
    }
    public String categoria() {
      return "Aplicar Fuerza";
    }
    public Mod_AtraccionAlCentro generarInstancia() {
      return new Mod_AtraccionAlCentro();
    }
  };
}
