class Monitor {
  ...
  void niveles(float a, float b, float t, int nivel) {
    ...
    colorMode(RGB);
    for (int i= 0; i<3; i++) {
      colorsitos[i]= paleta[3];
    }
    colorsitos[nivel] = paleta[1];
    noStroke();
  ...
  void eje(float a, float b, float t, int inclinacion) {
    ...
    if (inclinacion != 0 && inclinacion != 4) {
      colorsitos[inclinacion] = paleta[1];

      if (colorsitos[2]!=paleta[3]) {
        fill(colorsitos[2], 150);
        // -----------eje central
        for (int i= 0; i<2; i++) {
      ...
      colorsitos[inclinacion] = paleta[4];
